    ...
    try:
        ...
        if user_role == 'admin':
            ...
            try:
                ...
                # Since there's no status field, we'll use is_approved instead
                ...
                # Get role distribution - count by joining with role table
                from models.role_models import Role
                admin_role = Role.query.filter_by(role_name='admin').first()
                manager_role = Role.query.filter_by(role_name='manager').first()
                developer_role = Role.query.filter_by(role_name='developer').first()
                ...
                developer_count = User.query.filter_by(role_id=developer_role.role_id).count() if developer_role else 0
                viewer_count = User.query.filter_by(role_id=viewer_role.role_id).count() if viewer_role else 0
                
                # Get client role count
                client_role = Role.query.filter_by(role_name='client').first()
                client_count = User.query.filter_by(role_id=client_role.role_id).count() if client_role else 0
                ...
                dashboard_stats = {
                    ...
                    # Individual role counts for template
