...
def update_task_status(task_id):
    ...
    # Valid statuses (matching TaskStatus enum)
...
@login_required
def add_project_column(project_id):
    """API endpoint to add a new column/status to a project"""
    if request.method != 'POST':
        return jsonify({'error': 'Method not allowed'}), 405
    
    project = Project.query.get_or_404(project_id)
    
    # Check permissions - only admins and managers can add columns
    user_role = getattr(current_user, 'role_name', None)
    ...
    column_status = data.get('status')
    
    if not column_name or not column_status:
        return jsonify({'error': 'Column name and status are required'}), 400
    
    # For now, we'll just return success since we're using predefined statuses
    # In a full implementation, you might want to store custom columns in the database
    try:
        return jsonify({
            'success': True,
            'message': f'Column "{column_name}" with status "{column_status}" added successfully',
            'column': {
                'name': column_name,
